...
@onready var firepoint: Marker2D = $gunpivot/firepoint

@export var inventory: Array[GunResource] = []
var current_gun: GunResource
var current_index: int = 0

@onready var gun_sprite: Sprite2D = $gunpivot/gun
...
func fire_gun(direction: Vector2):
	...
	var bullet_data = current_gun.bullet_data
	bullet.setup(bullet_data)
	
func swap_gun(index: int):
	#print_debug("swapping to: " + current_gun.name)
	current_index = index
	current_gun = inventory[index]
	gun_sprite.texture = current_gun.sprite
